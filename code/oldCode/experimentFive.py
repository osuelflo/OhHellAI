...
def PlayManyRounds(data,numRounds,numPlayers,numTricks,numParticles,lamb,isAI,useHeuristic,mcIterations,selfish,aggs):
    for game in range(numRounds):
        ...
        scores, tricksWon,bids = PlayRound(numPlayers,numTricks,numParticles,lamb,dealer,isAI,useHeuristic,mcIterations,selfish,aggs)
        for i in range(numPlayers):
            ...
            data["aggressiveness"].append(aggs[i])

def PlayRound(numPlayers,numTricks,numParticles,lamb,dealer,isAI,useHeuristic,mcIterations,selfish,aggs):
    # if useHeuristic:
    state = OhHellHeurState(numPlayers, numTricks, numParticles, lamb, dealer, True, isAI,useHeuristic,aggs)
    # else:
...
if __name__ == "__main__":
    ...
    particles = [10,50,100,500]
    mcsims = [[],[],[1000,1000],[1000,1000,1000],[1000,100,10,1]]
    ...
    lambs = [1.01,1.05,1.1,1.3,1.6,2,3]
    lamb = 1.3
    count = 0
    aggressiveness = ["aggressive","passive","normal"]
    numParticles = 1000
    for numTricks in range(10,0,-1):
        for numPlayers in range(4,1,-1):
            mcIterations = [1000 for i in range(numPlayers)]
            # mcIterations[0] = 1000
            isAI = [True for i in range(numPlayers)]
            selfish = [True for i in range(numPlayers)]
            useHeuristic = [True for i in range(numPlayers)]
            aggs = ["normal" for i in range(numPlayers)]
            for numParticles in particles:
                for i in range(numPlayers+1):
                    for j in range(i):
                        selfish[j] = False
                    # aggs[0] = agg
                    PlayManyRounds(data,40,numPlayers,numTricks,numParticles,lamb,isAI,useHeuristic,mcIterations,selfish,aggs)
                    ...
                    if elapsed > 3600:
                        ...
                        df.to_csv(f"output{count}HrTwo.csv", index=False)
                        count += 1
                        data = {
                            "gameID": [],
                            "player": [],
